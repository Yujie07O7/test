...
class PPO:
    ...
    def train(self, verbose=False):
        ...
        gold_shares_history = []  # 記錄 gold 持股數量
        time_steps = []  # 記錄對應的日期時間
        vcit_shares_history = []  # 記錄 VCIT 持股數量
        time_steps = []  # 記錄對應的日期時間
        ...
        while True:
            ...
            while not done:
                ...
                # 記錄 VCIT 的持股數量 (假設 VCIT 是 tickers.txt 的最後一個資產)
                shares = self.env.get_shares()[-1]
                vcit_shares_history.append(shares)
                time_steps.append(info.date())  # 紀錄日期
                ...
                if verbose:
                    print(f"PPO training - Date: {info.date()}, Balance: {int(self.env.get_balance())}, "
                          f"Cumulative Return: {int(wealth) - 1000000}, VCIT Shares: {shares}")
        ...
        # **存檔 VCIT 持股數量數據**
        vcit_shares_df = pd.DataFrame({'Date': time_steps, 'VCIT Shares': vcit_shares_history})
        vcit_shares_df.to_csv(self.figure_dir + f'/{self.repeat}_vcit_shares.csv', index=False)
